class Drawer {
  ...
  createLineEquation(point1, point2) {
    ...
    let x1 = Math.round(point2.x);
    let y1 = Math.round(point2.y);
  ...
  snapshot() {
    this.currentPixels = this.ctx.getImageData(
      0,
      0,
      this.mouse.canvas.width,
      this.mouse.canvas.height,
    );
  }
  ...
  drawPoint(startPoint) {
    ...
    this.ctx.roundRect(
      startPoint.x - startPoint.lineWidth / 2,
      startPoint.y - startPoint.lineWidth / 2,
      startPoint.lineWidth,
      startPoint.lineWidth,
      startPoint.lineWidth / 2,
    );
